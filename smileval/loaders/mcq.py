...
class MCQQuestionAskExperiment(Experiment):
    ...
    def format_question(self, answer_choices = None, formatting_override = None) -> str:
        if not answer_choices:
            answer_choices = self.answer_choices
        qa_format = formatting_override if formatting_override else self.formatting
        return qa_format["question_answer_format"].format(self.question,mcq_templates.format_choices(answer_choices, qa_format["presentation_selection_type"]["choices"], qa_format["sep"]))
    ...
    async def execute(self, context: ExperimentContext) -> ExperimentOutcome:
        ...
        prompt = self.format_question(answer_choices)
        ...
        if not self.use_strict:
            ...
            # print("Correct symbols", correct_symbols, self.correct_answers, answer_choices.index(self.correct_answers[0]))
